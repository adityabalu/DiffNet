...
class NS_FPS_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        pc_stuff = np.array([self.pc, self.normals])

        forcing = np.ones_like(self.x)*(1/self.Re)
        return torch.FloatTensor(inputs), torch.FloatTensor(pc_stuff), torch.FloatTensor(forcing).unsqueeze(0)
...
class NS_FPS(DiffNet2DFEM):
    ...
    def apply_bc_pointcloud(self, pc, u):
        ...
        return u_pts, u_x_pts, u_y_pts

    def calc_residuals_stokes(self, pred, inputs_tensor, pc_tensor, forcing_tensor):
        ...
        u_pts, u_x_pts, u_y_pts = self.apply_bc_pointcloud(pc, u)
        v_pts, v_x_pts, v_y_pts = self.apply_bc_pointcloud(pc, v)
        ...
        loss1 = torch.norm(R1, 'fro') + torch.sum((u_pts-0.0)**2) 
        loss2 = torch.norm(R2, 'fro') + torch.sum((v_pts-0.0)**2) 
        loss3 = torch.norm(R3, 'fro')
    ...
    def loss(self, pred, inputs_tensor, pc_tensor, forcing_tensor):
        if self.eq_type == 'ns':
            R1, R2, R3 = self.calc_residuals_ns(pred, inputs_tensor, forcing_tensor)
        elif self.eq_type == 'stokes':
            R1, R2, R3 = self.calc_residuals_stokes(pred, inputs_tensor, pc_tensor, forcing_tensor)
    ...
    def forward(self, batch):
        inputs_tensor, pc_tensor, forcing_tensor = batch
        return self.net_u[0], self.net_v[0], self.net_p[0], inputs_tensor, pc_tensor, forcing_tensor

    def training_step(self, batch, batch_idx, optimizer_idx):
        u, v, p, inputs_tensor, pc_tensor, forcing_tensor = self.forward(batch)
        loss_vals = self.loss((u, v, p), inputs_tensor, pc_tensor, forcing_tensor)
    ...
    def on_epoch_end(self):
        ...
        self.net_v.eval()
        self.net_p.eval()
        inputs, pc, forcing = self.dataset[0]
        u, v, p, u_x_gp, v_y_gp = self.do_query(inputs, pc, forcing)
    ...
    def do_query(self, inputs, pc, forcing):
        u, v, p, inputs_tensor, pc_tensor, forcing_tensor = self.forward((inputs.unsqueeze(0).type_as(next(self.net_u.parameters())), pc.unsqueeze(0).type_as(next(self.net_u.parameters())), forcing.unsqueeze(0).type_as(next(self.net_u.parameters()))))
