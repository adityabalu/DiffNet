...
class Eikonal(DiffNet2DFEM):
    ...
    def forward(self, batch):
        ...
        u, latent = self.network(nu)
        return u, latent, inputs_tensor, forcing_tensor
    ...
    def training_step(self, batch, batch_idx):
        u, latent, inputs_tensor, forcing_tensor = self.forward(batch)
        ...
        if self.masking_type == 'WN':
            ...
            # loss_vae = F.kl_div(latent, torch.randn_like(latent))
    ...
    def do_query(self, inputs, forcing):
        ...
        # print(inputs.shape)
        # exit()
        u, _, _, _ = self.forward((inputs.type_as(next(self.network.parameters())), 
