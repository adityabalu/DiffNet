...
from DiffNet.networks.immdiff_networks import ImmDiff, IBN_DGCNN2d, ImmDiff_Large, eikonal_linear
...
class ParametricNURBS(data.Dataset):
    def __init__(self, data_path, domain_size=32):
        ...
        self.pc[:,:,1:2] += 0.5
        self.pc[:,:,0:1] += 0.25
        # x_translation = np.random.uniform(low=0.1, high=0.7, size=(2,1))
        # y_translation = np.random.uniform(low=0.3, high=0.7, size=(2,1))
    ...
    def __getitem__(self, index):
        ...
        boundary = self.bc2
        return torch.FloatTensor(inputs), torch.FloatTensor(forcing).unsqueeze(0), torch.FloatTensor(self.bc2)
...
class Eikonal(DiffNet2DFEM):
    ...
    def loss_eikonal(self, u, inputs_tensor, forcing_tensor):
        ...
        x_pts = pc[:,:,:,0:1] - nidx.type_as(pc)*self.hx 
        y_pts = pc[:,:,:,1:2] - nidy.type_as(pc)*self.hy
    ...
    def loss_poisson_eikonal(self, u, inputs_tensor, forcing_tensor, bc):
        N_values = self.Nvalues.type_as(u)
        ...
        u = torch.where(u_sdf<=0.0, ones, u) # irregular domain is source
        u = torch.where(bc2>0.5, zeros, u) # outside boundary of domain is sink
    ...
    def loss_poisson_windingnumber(self, u, inputs_tensor, forcing_tensor, bc2):
        N_values = self.Nvalues.type_as(u)
        dN_x_values = self.dN_x_values.type_as(u)
        dN_y_values = self.dN_y_values.type_as(u)
        gpw = self.gpw.type_as(u)
        ...
        ones = torch.ones_like(u)
        zeros = torch.zeros_like(u)
        u = torch.where(winding_number>0.1, ones, u) # irregular domain is source
        u = torch.where(bc2>0.5, zeros, u) # outside boundary of domain is sink
    ...
    def forward(self, batch):
        inputs_tensor, forcing_tensor, bc = batch
        nu = inputs_tensor[:,0:1,:,:2].squeeze(1)
        return self.network(nu), inputs_tensor, forcing_tensor, bc
    ...
    def training_step(self, batch, batch_idx):
        u, inputs_tensor, forcing_tensor, bc = self.forward(batch)
        if self.masking_type == 'eikonal':
            ...
            # loss_poisson = self.loss_poisson_eikonal(u, inputs_tensor, forcing_tensor, bc).mean()
            loss_val = loss_eikonal #+ loss_poisson*100
        if self.masking_type == 'WN':
            loss_poisson = self.loss_poisson_windingnumber(u, inputs_tensor, forcing_tensor, bc).mean()
    ...
    def do_query(self, inputs, forcing, bc):
        inputs = inputs.unsqueeze(1)
        forcing = forcing.unsqueeze(1)
        bc = bc.unsqueeze(1)
        batch = (inputs.type_as(next(self.network.parameters())), 
                forcing.type_as(next(self.network.parameters())),
                bc.type_as(next(self.network.parameters())))
        u, inputs_tensor, _, _ = self.forward(batch)
    ...
    def on_epoch_end(self):     
        if self.current_epoch % self.save_frequency == 0:
            self.network.eval()
            inputs_1, forcing_1, bc = self.dataset[0]
            ...
            if self.masking_type == 'WN':
                u, winding_number = self.do_query(inputs, forcing, bc)
                inputs = winding_number
            elif self.masking_type == 'eikonal':
                u = self.do_query(inputs_1, forcing_1, bc)
            self.plot_contours(u, inputs_1)
    ...
    def plot_contours(self, u, inputs):
        ...
        if self.masking_type == "eikonal":
            ...
            im0 = axs[0].imshow(u,vmin=0.0,vmax=1.0,cmap='jet')
            ...
            im0 = axs[0,0].imshow(u, vmin=0.0, vmax=1.0,cmap='jet')
            fig.colorbar(im0, ax=axs[0,0])

            im1 = axs[0,1].imshow(inputs,vmin=0.0, vmax=1.0,cmap='gray')
            fig.colorbar(im1, ax=axs[0,1])

            im2 = axs[1,0].imshow(u[1],vmin=0.0, vmax=1.0,cmap='jet')
            fig.colorbar(im2, ax=axs[1,0])

            im3 = axs[1,1].imshow(inputs[1],vmin=0.0, vmax=1.0,cmap='gray')
...
def main():
    ...
    masking_type = 'eikonal' # options: 'eikonal' OR 'WN' ie winding number
    batch_size = 1
    max_epochs = 2000
    if masking_type == 'WN':
        ...
        network = eikonal_linear()
    basecase = Eikonal(network, dataset, batch_size=batch_size, masking_type=masking_type, domain_size=domain_size, domain_length=1)
    log_string = 'eikonal_32'
