...
from DiffNet.networks.immdiff_networks import ImmDiff, IBN_DGCNN2d, ImmDiff_Large_normals
...
class Eikonal(DiffNet2DFEM):
    ...
    def forward(self, batch):
        ...
        pc = inputs_tensor[:,0:1,:,:2].squeeze(1)
        normals = inputs_tensor[:,0:1,:,2:4].squeeze(1)
        return self.network(pc, normals), inputs_tensor, forcing_tensor
...
def main():
    ...
    batch_size = 3
    opt_switch_epochs = 1000
    max_epochs = 1000
    if masking_type == 'WN':
        network = ImmDiff_Large_normals(out_channels=1)
        # network = IBN_DGCNN2d()
