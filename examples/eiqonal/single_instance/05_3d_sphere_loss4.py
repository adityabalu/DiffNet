...
class Eiqonal(DiffNet3DFEM):
    ...
    def loss(self, u, inputs_tensor, forcing_tensor):
        ...
        nidz = (pc[:,:,:,2]/self.hz).type(torch.LongTensor).to(pc.device)
    ...
    def loss4(self, u, inputs_tensor, forcing_tensor):
        ...
        dN_z_values = self.dN_z_values.type_as(f)
        gpw = self.gpw.type_as(u)
        ...
        R1 = torch.zeros_like(u); R1 = self.Q1_3D_vector_assembly(R1, R_split_1)
        ...
        normals_loss = torch.sum((u_x_pts*normals[:,:,:,0]+u_y_pts*normals[:,:,:,1]+u_z_pts*normals[:,:,:,2] - 1.0)**2)
